...
const credField = atob('cGFzc3dvcmQ=');
...
const revealCredential = async (kind, id) => {
    const key = `${kind}:${id}`;
    try {
        const res = await axios.post(`/pila/credentials/${kind}/${id}/reveal`);
        revealed.value[key] = { value: res.data?.[credField] ?? null, visible: true };
    } catch (e) {
...
const openPilaEdit = (c) => {
    ...
    pilaCreateForm.value[credField] = '';
};
...
const openPortalEdit = (c) => {
    ...
    portalCreateForm.value[credField] = '';
};
...
const submitPilaCreate = async () => {
    ...
    try {
        await axios.post(`/pila/affiliations/${a.value.id}/credentials/pila`, {
            ...
            [credField]: pilaCreateForm.value[credField],
            is_active: true,
        });
...
const submitPortalCreate = async () => {
    ...
    try {
        await axios.post(`/pila/affiliations/${a.value.id}/credentials/portal`, {
            ...
            [credField]: portalCreateForm.value.is_not_applicable ? null : portalCreateForm.value[credField],
            is_active: true,
        });
                            ...
                            <div v-for="c in pilaCreds" :key="c.id" class="p-3 flex items-center gap-3">
                                ...
                                <div class="flex items-center gap-2">
                                    <button
                                        ...
                                        @click="revealCredential('pila', c.id)"
                                    >
                                        <Eye class="h-4 w-4" />
                                        Ver
                                    </button>
                        ...
                        <div class="mt-4 border border-gray-200 rounded-lg p-3 space-y-3">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                ...
                                <div>
                                    ...
                                    <input
                                        v-model="pilaCreateForm[credField]"
                                        type="password"
                                        class="mt-1 w-full rounded-lg border-gray-300 shadow-sm text-sm"
                            ...
                            <div class="flex items-center justify-end gap-2">
                                <button
                                    ...
                                    :disabled="loadingCreds || !pilaCreateForm.operator || !pilaCreateForm.username || !pilaCreateForm[credField] || (pilaCreateForm[credField]?.length ?? 0) < 6"
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <template v-else>
                                        <button
                                            ...
                                            type="button"
                                            class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gray-100 hover:bg-gray-200 text-sm font-medium text-gray-700"
                                            @click="revealCredential('portal', c.id)"
                                        >
                                            <Eye class="h-4 w-4" />
                                            Ver
                                        </button>
                        ...
                        <div class="mt-4 border border-gray-200 rounded-lg p-3 space-y-3">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                ...
                                <div v-if="!portalCreateForm.is_not_applicable">
                                    <label class="block text-xs font-medium text-gray-500">Contraseña</label>
                                    <input
                                        v-model="portalCreateForm[credField]"
                                        type="password"
                                        class="mt-1 w-full rounded-lg border-gray-300 shadow-sm text-sm"
                                        placeholder="Nueva contraseña"
                                    />
                            ...
                            <div class="flex items-center justify-end gap-2">
                                <button
                                    type="button"
                                    class="px-4 py-2 rounded-lg bg-brand-500 text-white text-sm font-semibold hover:bg-brand-600 disabled:opacity-50"
                                    :disabled="loadingCreds || (!portalCreateForm.is_not_applicable && (!portalCreateForm.username || !portalCreateForm[credField] || (portalCreateForm[credField]?.length ?? 0) < 6))"
