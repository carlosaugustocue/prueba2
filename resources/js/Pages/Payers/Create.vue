    ...
    <AppLayout>
        <div class="max-w-2xl mx-auto space-y-6">
            <div>
                ...
                <h1 class="mt-2 text-2xl font-bold text-gray-900">Nuevo pagador</h1>
                <p class="mt-1 text-sm text-gray-500">Registre quién paga la seguridad social (NIT o documento)</p>
            </div>

            <form @submit.prevent="form.post('/payers')" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-5">
                <div class="flex items-center gap-3 pb-4 border-b border-gray-200">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500">NIT o documento único por pagador</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input id="document_number" v-model="form.document_number" type="text" required class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-brand-500 focus:ring-brand-500" placeholder="Ej. 900123456-1" />
                ...
                <div class="flex flex-wrap gap-3 pt-4 border-t border-gray-200">
                    <button type="submit" :disabled="form.processing" class="inline-flex items-center justify-center px-4 py-2 font-medium rounded-lg bg-brand-500 text-white hover:bg-brand-600 disabled:opacity-50">
                        {{ form.processing ? 'Guardando...' : 'Crear pagador' }}
                    </button>
                    <Link href="/payers" class="inline-flex items-center justify-center px-4 py-2 font-medium rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50">
                        Cancelar
                    </Link>
                </div>
            </form>
