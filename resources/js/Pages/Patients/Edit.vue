...
const currentHolder = computed(() => patientData.value.holder || null);
const hasBeneficiaries = computed(() => patientData.value.beneficiaries_count > 0);
...
const form = useForm({
    ...
    second_name: patientData.value.second_name || '',
    last_name: patientData.value.last_name || '',
    second_last_name: patientData.value.second_last_name || '',
    phone: patientData.value.phone || '',
    phone_2: patientData.value.phone_2 || '',
    whatsapp: patientData.value.whatsapp || '',
    email: patientData.value.email || '',
    address: patientData.value.address || '',
    neighborhood: patientData.value.neighborhood || '',
    eps_id: patientData.value.eps?.id || patientData.value.eps_id || '',
    afp_name: patientData.value.afp_name || '',
    arp_name: patientData.value.arp_name || '',
    arp_risk_class: patientData.value.arp_risk_class || '',
    patient_type: patientData.value.patient_type || '',
    holder_id: patientData.value.holder_id || '',
    relationship_type: patientData.value.relationship_type || '',
    birth_date: patientData.value.birth_date || '',
    gender: patientData.value.gender || '',
    status: patientData.value.status || 'ACTIVO',
...
<template>
    <AppLayout>
        <div class="max-w-3xl mx-auto">
            <div class="mb-8">
                <Link :href="`/patients/${patientData.id}`" class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-brand-600 transition-colors">
                    <ChevronLeft class="h-4 w-4" />
                    Volver al paciente
                </Link>
                <h1 class="text-3xl font-bold text-gray-900 mt-3">Editar Paciente</h1>
                <p class="text-gray-500 mt-1">{{ patientData.full_name }}</p>
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div v-if="hasBeneficiaries && patientData.patient_type === 'cotizante'" class="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-xl">
                        <p class="text-amber-800 text-sm">
                            <strong>Nota:</strong> Este paciente tiene beneficiarios asociados. No puede cambiar a beneficiario mientras tenga beneficiarios.
                        </p>
                    </div>
                ...
                <div class="flex flex-col sm:flex-row justify-end gap-4 pt-4">
                    <Link :href="`/patients/${patientData.id}`" class="inline-flex items-center justify-center gap-2 px-6 py-3 font-medium rounded-xl bg-white text-gray-700 border border-gray-300 hover:bg-gray-50 transition-colors">
                        <X class="h-5 w-5" />
                        Cancelar
                    </Link>
